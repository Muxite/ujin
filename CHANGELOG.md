...
- **docs(sync)**: Comprehensive audit of `README.md` and all `docs/` pages against the shipped 0.16.0 surface. Added "GraphQL endpoints" and "browser-driven interaction recipes" to the README intro feature list — both `GraphQLPollable` and `BrowserPollable` have shipped in `ujin.poll` (since 0.15.0 and 0.5.0 respectively) but were absent from the top-level feature pitch. Added `BrowserPollable` to the README "How it works → Roles" list alongside the other pollable classes. Confirmed: all 12 built-in transform kinds (`select`, `regex`, `template`, `dedupe`, `chunk`, `flatten`, `sort`, `limit`, `rename`, `aggregate`, `unique`, `fill`) are documented in `docs/JOBS.md` and the eight list-reshaping variants are documented in `docs/LIST_TRANSFORMS.md`; all nine source kinds (`http`, `rss`, `api`, `graphql`, `command`, `site`, `scrape`, `browser`, `plugin:<name>`) are in `docs/JOBS.md`; all extract modes (`links`, `article`, `auto`, `combined`, `structured`, `tables`, `images`, `metadata`, plus `html` for multi-extract) are in `docs/API.md` and `README.md`; all 12 CLI subcommands including `learned`, `doctor`, and `init` are in the README CLI section; adaptive-learning and robots features are in the quickstart, the "How it works" section, and `docs/ADAPTIVE.md` (which includes the `ujin learned` reference with `--host`/`--strategy-db`/`--json` flags). All runnable examples verified against the current API.

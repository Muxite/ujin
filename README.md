...
periodic load spikes. ujin backs off targets that aren't changing, speeds up ones
...
ujin serve targets.yaml     # run the engine as a daemon
ujin api [targets.yaml]     # REST + WebSocket service on :8900
```

## Service (REST + WebSocket)

`ujin.service` drives the engine over HTTP and streams change events:
...
- **Roles** (`ujin.poll`): `HttpPollable` (conditional GET + body fingerprint),
...
- **Adaptive** (`ujin.adapt.interval`): `AdaptiveInterval` grows the interval when
  unchanged, shrinks it when changed, clamped to `[min, max]`.
- **Jitter / stability** (`ujin.adapt.jitter`, `ujin.adapt.concurrency`): full /
  equal / decorrelated jitter + randomized initial phase, and a `TokenBucket` +
  AIMD limiter that flatten bursts.
- **Resilience** (`ujin.adapt.backoff`): exponential `Backoff` (honors
  `Retry-After`) and a `CircuitBreaker` that skips dead targets until a probe.
- **Engine** (`ujin.engine.PollEngine`): per-target adaptive state, jittered
  scheduling, global smoothing, change callbacks, `stats()`. `clock`/`rng`/`sleep`
  are injectable for deterministic tests.

The scrape toolkit (`ujin.fetch`, `ujin.extract`, `ujin.cache`, `ujin.sources`)
...
pip install -e ".[yaml]"    # + `ujin serve/sweep` config loading
```
